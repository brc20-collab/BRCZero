...
func getAccountFromMpt(height uint64) map[string]interface{} {
	...
	for itr.Next() {
		...
		if ethAcc, ok := acc.(*apptypes.EthAccount); ok {
			var brc10Acc = TempNewAccountPretty{Storage: make(map[string]string)}
			err = json.Unmarshal(buff, &brc10Acc)
			panicError(err)
			...
			if !bytes.Equal(ethAcc.CodeHash, mpt.EmptyCodeHashBytes) {
				...
				for cItr.Next() {
					...
					brc10Acc.Storage[prefixKey.String()] = hex.EncodeToString(cItr.Value)
				}
			}

			result[acc.GetAddress().String()] = &brc10Acc
		} else {
