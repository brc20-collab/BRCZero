...
import (
	...
	"github.com/brc20-collab/brczero/x/erc20/types"
...
const (
	SendToIbcEventName         = "__brczeroSendToIbc"
	SendNative20ToIbcEventName = "__brczeroSendNative20ToIbc"
	SendToWasmEventName        = "__brczeroSendToWasm"
)

// SendToIbcEvent represent the signature of
// `event __brczeroSendToIbc(string recipient, uint256 amount)`
var SendToIbcEvent abi.Event

// SendNative20ToIbcEvent represent the signature of
// `event __brczeroSendNative20ToIbc(string recipient, uint256 amount, string portID, string channelID)`
...
func (h SendToIbcEventHandler) Handle(ctx sdk.Context, contract common.Address, data []byte) error {
	...
	// to avoid unpacking any contract '__brczeroSendToIbc' event, which consumes performance
...
func (h SendNative20ToIbcEventHandler) Handle(ctx sdk.Context, contract common.Address, data []byte) error {
	...
	// first confirm that the contract address and denom are registered,
	// to avoid unpacking any contract '__brczeroSendNative20ToIbc' event, which consumes performance
	denom, found := h.Keeper.GetDenomByContract(ctx, contract)
	if !found {
		return fmt.Errorf("contract %s is not connected to native token", contract)
	}
