...
import (
	...
	cosmos_context "github.com/brc20-collab/brczero/libs/cosmos-sdk/client/context"
	...
	"github.com/brc20-collab/brczero/app/rpc/types"
...
const (
	...
	defaultChainID         = 65
	defaultMinGasPrice     = "0.0000000001brc10"
	safeLowGP              = "0.0000000001brc10"
	avgGP                  = "0.0000000001brc10"
	fastestGP              = "0.00000000015brc10"
...
func (suite *RPCTestSuite) SetupTest() {
	...
	chainId := apptesting.GetbrczeroID(1)
...
func (suite *RPCTestSuite) TestEth_GetBalance() {
	// initial balance of hexAddr2 is 1000000000brc10 in test.sh
...
func (suite *RPCTestSuite) TestEth_GasPrice() {
	...
	// min gas price in test.sh is "0.000000001brc10"
...
func (suite *RPCTestSuite) TestEth_SendTransaction_Transfer() {
	...
	suite.Require().NotNil(receipt)
	suite.Require().Equal("0x1", receipt["status"].(string))
	//suite.T().Logf("%s transfers %sbrc10 to %s successfully\n", hexAddr1.Hex(), value.String(), receiverAddr.Hex())
	...
	//suite.Require().NotNil(receipt)
	//suite.Require().Equal("0x1", receipt["status"].(string))
	//suite.T().Logf("%s transfers %sbrc10 to %s successfully with nil gas price \n", hexAddr1.Hex(), value.String(), receiverAddr.Hex())
...
func (suite *RPCTestSuite) TestEth_GetBlockByHash() {
	...
	// TODO: brczero only supports the block query with txs' hash inside no matter what the second bool argument is.
