...
func TestParseMsgWrapper(t *testing.T) {
	testcases := []struct {
		...
		{
			input: `{"type": "brczero/staking/MsgDeposit","value": {"delegator_address": "0x4375D630687C83471829227b5C1Ea92217FD6265","quantity": {"denom": brc10,"amount": "1"}}}`,
			fnCheck: func(ret *MsgWrapper, err error) {
				require.NoError(t, err)
				require.Equal(t, "brczero/staking/MsgDeposit", ret.Name)
				require.Equal(t, "{\"delegator_address\": \"0x4375D630687C83471829227b5C1Ea92217FD6265\",\"quantity\": {\"denom\": \"brc10\",\"amount\": \"1\"}}", string(ret.Data))
			},
		},
		{
			input: `{"type": "brczero/staking/MsgWithdraw","value": {"delegator_address": "0x4375D630687C83471829227b5C1Ea92217FD6265","quantity": {"denom": brc10,"amount": "1"}}}`,
			fnCheck: func(ret *MsgWrapper, err error) {
				require.NoError(t, err)
				require.Equal(t, "brczero/staking/MsgWithdraw", ret.Name)
				require.Equal(t, "{\"delegator_address\": \"0x4375D630687C83471829227b5C1Ea92217FD6265\",\"quantity\": {\"denom\": \"brc10\",\"amount\": \"1\"}}", string(ret.Data))
			},
		},
		// error
		{
			input: `{"type1": "brczero/staking/MsgWithdraw","value":""}`,
			fnCheck: func(ret *MsgWrapper, err error) {
				require.NotNil(t, err)
			},
		},
		{
			input: `{"type": "brczero/staking/MsgWithdraw","value1":"123"}`,
			fnCheck: func(ret *MsgWrapper, err error) {
				require.NotNil(t, err)
			},
		},
