...
import (
	...
	tmcli "github.com/brc20-collab/brczero/libs/tendermint/libs/cli"
...
func PostCommands(cmds ...*cobra.Command) []*cobra.Command {
	for _, c := range cmds {
		...
		c.Flags().String(FlagFees, "", "Fees to pay along with transaction; eg: 0.1brc10")
		c.Flags().String(FlagGasPrices, "", "Gas prices to determine the transaction fee (e.g. 0.1brc10)")
		c.Flags().String(FlagNode, "tcp://localhost:26657", "<host>:<port> to tendermint rpc interface for this chain")
		c.Flags().Bool(FlagUseLedger, false, "Use a connected Ledger device")
...
func AddTxFlagsToCmd(cmd *cobra.Command) {
	...
	cmd.Flags().String(FlagFees, "", "Fees to pay along with transaction; eg: 0.1brc10")
	cmd.Flags().String(FlagGasPrices, "", "Gas prices in decimal format to determine the transaction fee (e.g. 0.1brc10)")
	cmd.Flags().String(FlagNode, "tcp://localhost:26657", "<host>:<port> to tendermint rpc interface for this chain")
